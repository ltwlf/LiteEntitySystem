...
namespace LiteEntitySystem.Extensions
{
    public class SyncNetSerializable<T> : SyncableField<T> where T : INetSerializable
    {
        ...
        protected internal override void OnSyncRequested()
        {
            ...
            // Write uncompressed length into first 2 bytes
            FastBitConverter.GetBytes(CompressionBuffer, 0, (ushort)WriterCache.Length);

            // Compress the serialized data to the remainder of the buffer
        ...
        /// <summary>
        /// Called on the client when the server updates the data.
        /// Decompresses, deserializes into _value, and fires OnValueChanged if changed.
        /// </summary>
        private void Init(ReadOnlySpan<byte> data)
        {
            ...
            // Decompress into CompressionBuffer
            LZ4Codec.Decode(data.Slice(2), new Span<byte>(CompressionBuffer));
